				...
				try {
					...
					if (error.message?.includes("Cannot create property")) {
						...
						Reflect.set(
							target,
							key,
							createDeepProxy(emptyObj, instance, propertyKey, triggerPersist)
						);
 ...
 */
export function Persist<T, V>(
	target: undefined,
	context: ClassFieldDecoratorContext<T, V>
): void;
export function Persist(
	target: any,
	propertyKeyOrContext: string | ClassFieldDecoratorContext<any, any>
): void {
	...
	if (typeof propertyKeyOrContext === "string") {
		...
		context.addInitializer(function () {
			...
			const triggerPersist = () => {
				const currentValue = (instance as any)[PERSISTED_VALUES].get(
					propertyKey
				);
				...
				if ((instance as any).storage?.raw) {
					...
					(instance as any)
						._persistProperty(propertyKey, currentValue)
						.catch((err: Error) => {
			...
			Object.defineProperty(instance, propertyKey, {
				...
				set(value: any) {
					...
					if ((this as any).storage?.raw) {
						// Store the value in the database
						(this as any)
							._persistProperty(propertyKey, proxiedValue)
							.catch((err: Error) => {
								console.error(
									`Failed to persist property ${propertyKey}:`,
									err
								);
							});
					}
 ...
 */
export async function initializePersistedProperties(
	instance: any
): Promise<void> {
	...
	try {
		...
		await instance.storage.__studio({
			type: "query",
			statement:
				"CREATE TABLE IF NOT EXISTS _actor_persist (property TEXT PRIMARY KEY, value TEXT)",
		});
				...
				try {
					...
					try {
						...
						if (parsedValue && parsedValue.__error === "Serialization failed") {
							console.warn(
								`Property ${row.property} had serialization issues: ${parsedValue.value}`
							);
							// Use an empty object as fallback for previously failed serializations
							parsedValue =
								typeof parsedValue.value === "string" ? parsedValue.value : {};
						}
						...
						if (
							...
							if (typeof parsedValue !== "object" || parsedValue === null) {
								console.warn(
									`Property ${row.property} type mismatch: expected object, got ${typeof parsedValue}. Resetting to initial structure.`
								);
						...
						console.error(
							`Failed to parse persisted value for ${row.property}:`,
							parseErr
						);
					...
					const triggerPersist = () => {
						...
						if (instance.storage?.raw) {
							// Store the value in the database
							instance
								._persistProperty(row.property, currentValue)
								.catch((err: Error) => {
									console.error(
										`Failed to persist property ${row.property}:`,
										err
									);
								});
						}
					...
					console.error(
						`Failed to process persisted value for ${row.property}:`,
						err
					);
...
export async function persistProperty(
	...
	try {
		...
		await instance.storage.__studio({
			type: "query",
			statement:
				"INSERT INTO _actor_persist (property, value) VALUES (?, ?) ON CONFLICT(property) DO UPDATE SET value = ?",
		...
		if (typeof instance.onPersist === "function") {
			try {
				...
				console.error(
					`Error in onPersist hook for property ${propertyKey}:`,
					hookErr
				);
			}
		}
