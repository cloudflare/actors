...
export abstract class Actor<E> extends DurableObject<E> {
	 ...
	 */
	static get<T extends Actor<any>>(
		this: new (
			state: ActorState,
			env: any
		) => T,
	...
	constructor(ctx?: ActorState, env?: E) {
		if (ctx && env) {
			...
			// @ts-expect-error - This is handled internally by the framework
			super();
		}
	...
	private async _waitForSetName(): Promise<void> {
		...
		while (!this._setNameCalled) {
			...
			if (elapsed > timeout) {
				throw new Error(
					`setName() was not called within ${timeout}ms. Actor may not be properly initialized.`
				);
			}
	...
	async fetch(request: Request): Promise<Response> {
		...
		if (
			url.pathname === upgradePath ||
			url.pathname.startsWith(`${upgradePath}/`)
		) {
	 ...
	 */
	protected async onAlarm(_alarmInfo?: AlarmInvocationInfo) {
		// Default implementation is a no-op
	}
	 ...
	 */
	protected onPersist(_key: string, _value: any) {
		// Default implementation is a no-op
	}
	...
	protected onWebSocketMessage(_ws: WebSocket, _message: any) {
		// Default implementation is a no-op
	}
	...
	async track(idString: string) {
		if (TRACKING_ACTOR_NAME === idString) {
			throw new Error(
				`Cannot track instance with same name as tracking instance, change value to differ from "${TRACKING_ACTOR_NAME}"`
			);
		}

		const trackingStub = getActor(
			this.constructor as ActorConstructor<Actor<E>>,
			TRACKING_ACTOR_NAME
		) as unknown as Actor<E>;
		const currentDateTime = new Date().toISOString();
		await trackingStub.__studio({
			type: "query",
			statement:
				"CREATE TABLE IF NOT EXISTS actors (identifier TEXT PRIMARY KEY, last_accessed TEXT)",
		});
		await trackingStub.__studio({
			type: "query",
			statement: `INSERT INTO actors (identifier, last_accessed) VALUES (?, ?) ON CONFLICT(identifier) DO UPDATE SET last_accessed = ?`,
			params: [idString, currentDateTime, currentDateTime],
		});
	}
	...
	async destroy(_?: { forceEviction?: boolean }) {
		...
		if (this.name) {
			try {
				...
				console.error(
					`Failed to delete actor from tracking instance: ${e instanceof Error ? e.message : "Unknown error"}`
				);
			}
		}
		...
		const worker = {
			async fetch(
				...
				try {
					...
					if (idString === undefined) {
						return new Response(
							JSON.stringify({
								...
								message: "Invalid actor identifier",
							}),
							{
								status: 500,
								headers: {
					...
					const stub = getActor(
						ObjectClass as ActorConstructor<Actor<E>>,
						idString
					);
					...
					if (opts?.track?.enabled) {
						try {
							...
							console.error(
								`Failed to track actor instance: ${error instanceof Error ? error.message : "Unknown error"}`
							);
						}
					}
...
export function getActor<T extends Actor<any>>(
	...
	if (!bindingName) {
		throw new Error(
			`No Durable Object binding found for actor class ${className}. Check update your wrangler.jsonc to match the binding "name" and "class_name" to be the same as the class name.`
		);
	}
